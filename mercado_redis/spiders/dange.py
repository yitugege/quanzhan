...
class QuotesSpider(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        masvendidos = response.xpath(
            '//div[@class="ui-pdp-promotions-pill-label best_seller_position ui-pdp-background-color--WHITE ui-pdp-color--BLUE ui-pdp-size--XXSMALL ui-pdp-family--SEMIBOLD"]/a[@class="ui-pdp-promotions-pill-label__target"]/text()').get()
        print(masvendidos)
        if (masvendidos != None):
            category = masvendidos
        else:
            category = response.xpath(
                '//li[@class="andes-breadcrumb__item"][1]/a[@class="andes-breadcrumb__link"]/@title').get()
        # 链接
        url = response.url
     ...
     # 获取价格 没有价格删除连接
        ...
        like_count = response.xpath(
            '//span[@class="ui-pdp-review__amount"]/text()').get()
        print("-----------------------------------likeaccount--------------------------")
        print(like_count)
        ...
        Num_sell = response.xpath(
            '//div[@class="ui-pdp-header"]/div[@class="ui-pdp-header__subtitle"]/span[@class="ui-pdp-subtitle"]/text()').get()
        print("-----------------------------------Num_sell--------------------------")
        print(Num_sell)
        if Num_sell is None:
            return
        ...
        # print(type(Num_sell))
            ...
            pass
        # 获取60天销量
        days60_sell = response.xpath(
            '//strong[@class="ui-pdp-seller__sales-description"]/text()').get()
