...
class MercadoRedisPipeline:
    ...
    def process_item(self, item, spider):
       ...
       #判断如果存在则更新，如果不存在则插入
       sql = "INSERT INTO quanzhan(title,url,price,id,seller,like_count,Num_sell,time,days60_sell) VALUES(%s,%s,%s,%s,%s,%s,%s,%s,%s) ON DUPLICATE KEY UPDATE Num_sell=%s,title=%s,url=%s,price=%s,seller=%s,like_count=%s,time=%s,days60_sell=%s"
       try:
